...
int tileLoadPng(struct opengles *gles, struct tile *tile, const char *file)
{
	...
	if (png_file == 0)
	{
		fprintf(stderr, "loadPngTile.fopen('%s'): %s\n",
		        file, strerror(errno));
		exit(-1);
	}
	...
	if (ret < 0)
	{
		fprintf(stderr, "loadPngTile.fread('%s'): %s\n",
		        file, strerror(errno));
		exit(-1);
	}
	...
	// Maximale Mögliche Texturgröße auslesen
	uint32_t texsize;
	glGetIntegerv(GL_MAX_TEXTURE_SIZE, (int32_t*)&texsize);

	// Textur zu breit? Warnung ausgeben.
	if (width > texsize) {
		fprintf(stderr, "WARNUNG: PNG '%s' ist zu breit!"
			" (Max: %d, PNG: %d)\n", file, texsize, (int)width);
	}

	// Textur zu hoch? Warnung ausgeben.
	if (height > texsize) {
		fprintf(stderr, "WARNUNG: PNG '%s' ist zu hoch!"
			" (Max: %d, PNG: %d)\n", file, texsize, (int)height);
	}

	// Breite des PNG im Detail kontrollieren (width == 2^x?)
	for (ret = 0, i = width; i; i >>= 1) { if (i & 1) ret += 1; }
	if (ret > 1) {
		fprintf(stderr, "WARNUNG: PNG '%s' hat keine Zweierpotenz"
			" als Breite! (%d)\n", file, (int)width);
	}

	// Höhe des PNG im Detail kontrollieren (height == 2^x?)
	for (ret = 0, i = height; i; i >>= 1) { if (i & 1) ret += 1; }
	if (ret > 1) {
		fprintf(stderr, "WARNUNG: PNG '%s' hat keine Zweierpotenz"
			" als Höhe! (%d)\n", file, (int)width);
	}
