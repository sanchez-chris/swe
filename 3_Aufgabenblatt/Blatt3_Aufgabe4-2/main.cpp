...
int main (void)
{
	...
	int fd_can;
	fd_can = open("./test.bin", O_RDONLY, 0660);
...
/*
	...
	frame3.data[0] = 0x18;    // Datenbyte 1
	frame3.data[1] = 0x19;    // Datenbyte 2
	frame3.data[2] = 0x20;    // ..
	frame3.data[3] = 0x21;
*/
...
/*
	int ret = read(fd_can, &frame, sizeof(struct can_frame));

	cout << ret << endl;


	int ret2 = read(fd_can, &frame2, sizeof(struct can_frame));

	cout << ret2 << endl;


	int ret3 = read(fd_can, &frame3, sizeof(struct can_frame));
